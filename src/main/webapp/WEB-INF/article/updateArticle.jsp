...
<section class="container-fluid">
  ...
  <div class="row">
    ...
    <div class="col-sm-9 col-sm-offset-3 col-md-10 col-lg-10 col-md-offset-2 main" id="main">
      <div class="row">
        <form action="/blog/article/saveOrUpdateArticle"
                  ...
                  <select name="cid" id="category" class="form-control">
                      <c:forEach items="${categories}" var="category">
                          <option value="${category.cid}"
                                  <c:if test="${article.cid == category.cid}">selected</c:if>
                          >${category.cname}</option>
                      </c:forEach>
                  </select>
                  <label id="tags" style="margin-top: 10px">
                      <c:forEach items="${categories}" var="category">
                          <c:if test="${article.cid == category.cid}">
                              <c:forEach items="${category.tags}" var="tag">
                                  <input type="checkbox" name="tid" />${tag.tname}&nbsp;&nbsp;&nbsp;
                              </c:forEach>
                          </c:if>
                      </c:forEach>
                  </label>
            ...
            <div class="form-group">
              ...
              <div id="test-editor">
                <textarea name="content" id="content" style="display:none;">
                  ...
                  <%--<textarea name="content" id="contentText" style="display:none;">--%>
              ...
              <div class="add-article-box">
                  ...
                  <div class="add-article-box-content">
                      ...
                      <p><label>公开度：</label><input type="radio" name="isOpen" value="1"
                              <c:if test="${article.isOpen == 1}">checked</c:if> />公开
                          <input type="radio" name="isOpen" value="0"
                                 <c:if test="${article.isOpen == 0}">checked</c:if> />私密
                      </p>
                  ...
                  <div class="add-article-box-footer">
                      <button class="btn btn-primary" type="submit" onclick="test()">发布</button>
                  </div>
              </div>
          </div>
...
<script src="/blog/ueditor/ueditor.config.js"></script>
<script src="/blog/ueditor/ueditor.all.min.js"> </script>
<script src="/blog/ueditor/lang/zh-cn/zh-cn.js"></script>
<script src="/blog/js/admin-scripts.js"></script>
<%--Editormd--%>
...
<script>
    ...
    $(function() {
        editor = editormd("test-editor", {
            ...
            imageUploadURL : "/blog/fileUpload",//后端上传图片服务地址
        });
    });
